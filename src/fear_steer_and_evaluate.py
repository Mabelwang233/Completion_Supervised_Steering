"""
Fear-concept version of steer_and_evaluate.py -- same steering logic
(all layers, unit-norm directions, single global alpha), but judged with
fear_judge.judge_response(fear, question, response) instead of the
Shakespeare judge, since the fear judge needs to know WHICH fear is the
intended target, not just the question.

Unlike Shakespeare's 85-prompt test set, the fear test set is the fixed
list of 50 generic fear-themed questions -- no held-out statements,
matching the paper's design (statements are only ever used for training
here).

Supports up to 7 methods total (matching persona_steer_and_evaluate.py):
mean_difference + rfm (from baseline_path), sequence_rfm (from
seqrfm_path, label overridable via seqrfm_method_name), completion_rfm,
completion_diff_means, bag_of_words_rfm, bag_of_words_diff_means -- all
optional, so this can also be used to evaluate a single method in
isolation (e.g. a linear_late ablation) by omitting the others.
"""
import argparse
import csv
import json
...
def run_evaluation(dataset_path, baseline_path, seqrfm_path, out_dir, model_name,
    ...
    with open(rows_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["fear", "method", "alpha", "question", "response", "score", "explanation"])

        for method, per_layer_directions in directions.items():
            for alpha in alpha_grid:
                print(f"\n=== fear={fear}  method={method}  alpha={alpha} ===")
                for ex in tqdm(test_examples):
                    ...
                    row = [fear, method, alpha, ex["question"], response, score, explanation]
                    writer.writerow(row)
    ...
    with open(summary_path, "w", newline="") as f:
        ...
        for (method, alpha), scores in sorted(agg.items()):
            writer.writerow([fear, method, alpha, sum(scores) / len(scores), len(scores)])
