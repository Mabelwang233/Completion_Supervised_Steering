...
completion-RFM if --init_from_completion_rfm is provided (recommended),
diff-means if --init_from_diff_means is provided, else random.
completion-RFM takes priority over diff-means if both are set.
...
def train(args):
    ...
    # Priority: completion_rfm > diff_means > random
    raw_directions = {}
    if args.init_from_completion_rfm and os.path.exists(args.init_from_completion_rfm):
        import pickle
        with open(args.init_from_completion_rfm, "rb") as f:
            crfm = pickle.load(f)
        crfm_dirs = crfm.get("directions_per_layer", {})
        crfm_dirs = {int(k) % n_layers: v for k, v in crfm_dirs.items()}
        for layer in layers:
            v = F.normalize(crfm_dirs[layer].float().clone().to(device), dim=0) if layer in crfm_dirs \
                else F.normalize(torch.randn(d, device=device), dim=0)
            raw_directions[layer] = nn.Parameter(v)
        print("Initialized D from completion-RFM directions.")
    elif args.init_from_diff_means and os.path.exists(args.init_from_diff_means):
        import pickle
    ...
    out_dir = os.path.join(args.out_dir, concept.lower().replace(" ", "_"))
...
def main():
    ...
    p.add_argument("--init_from_diff_means",    default=None,
                   help="path to baseline_directions.pkl; initializes D from mean-difference direction")
    p.add_argument("--init_from_completion_rfm", default=None,
                   help="path to completion_rfm_directions.pkl; initializes D from completion-RFM "
                        "direction (takes priority over --init_from_diff_means if both are set)")
