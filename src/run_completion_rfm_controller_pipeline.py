"""
Completion-RFM + controller pipeline, k=1 only.
...
def run_one(concept_name, dataset_path, old_strategy_dir, new_strategy_dir, args):
    ...
    if not os.path.exists(completions_path):
        print(f"  SKIP {concept_name}: missing dataset_with_completions.json under {old_strategy_dir}")
        return None
...
def main(args):
    ...
    combined_path = os.path.join(
        args.new_output_dir, f"combined_summary_controller_{args.concept_type}_completion_rfm.csv")
