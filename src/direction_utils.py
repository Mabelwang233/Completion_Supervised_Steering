...
def get_hidden_states(prompts, model, tokenizer, hidden_layers, forward_batch_size, rep_token=-1, all_positions=False):
    ...
    if seq2seq:
        ...
        # Keep attention_mask as Long (int64); do NOT cast to half/float16 --
        # bfloat16 models (Qwen, Gemma) accept integer masks and reject float16 ones.
        # The original .half() here caused silent failures on non-Llama models.
