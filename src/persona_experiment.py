"""
Runs the full pipeline independently for EACH persona concept in the
manifest -- matching Fig 4's per-concept design, where "persona of X"
and "persona of Y" get entirely separate directions, not one shared
"persona" direction.

Now supports --completion_strategy, comparing two ways of handling
completion length:

  short_instructed_cut   -- add a length instruction to TRAINING prompts
                             too (e.g. "Answer in 15 words or less."),
                             and hard-cap generation at that length. The
                             model is told to finish a complete thought
                             within budget.
  long_subsampled_anchor -- no instruction, let the model generate a
                             full-length natural response, then anchor-
                             subsample down to the same effective count.
                             Avoids the model getting cut off mid-thought,
                             at the cost of more generation compute.
...
namespaced into a per-strategy subdirectory. baselines (mean_difference,
rfm) are the ONE exception: they only ever read the prompt, never touch
completions, so they're strategy-independent and computed ONCE per
concept and shared across both strategy runs.

Trains 7 methods per (concept, strategy): mean_difference, rfm (shared
baselines) + completion_rfm, completion_diff_means, bag_of_words_rfm,
bag_of_words_diff_means, sequence_rfm (strategy-specific).
"""
import argparse
import json
import os
...
def run_one_concept(persona, dataset_path, out_dir, args):
    ...
    # shared across both strategies -- never touches completions
    ...
    print(f"\n{'='*60}\nPERSONA: {persona}  |  STRATEGY: {args.completion_strategy}\n{'='*60}")

    print("\n[1/9] train baselines (RFM, diff-means, all layers -- shared across strategies)")
    ...
    print(f"\n[2/9] generate completions (train, max {args.train_completion_tokens} tokens, "
    ...
    print(f"\n[9/9] steer + evaluate (alpha sweep, persona judge, "
...
def main(args):
    ...
    # combine all per-concept summaries into one file (per strategy)
    import csv
    combined_path = os.path.join(args.output_dir, f"combined_summary_{args.completion_strategy}.csv")
...
if __name__ == "__main__":
    ...
    p.add_argument("--eval_gen_tokens", type=int, default=100,
                    help="max new tokens when generating TEST-TIME responses to be judged. "
                         "Keep generous so valid responses aren't truncated before judging.")
