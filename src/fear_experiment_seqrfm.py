"""
fear_experiment_seqrfm.py

Runs ONLY sequence-RFM and bag-of-words RFM for each fear concept in
the manifest, both with truncate_at_end=True and fast_bow=True.

Mirrors persona_experiment_seqrfm.py exactly -- only the judge and
dataset schema differ (fear_steer_and_evaluate / "fear" field).
...
Steps per concept:
  ...
  [5/5] steer + evaluate          -- sequence_rfm + bag_of_words_rfm only
        ...
        Results APPENDED to existing summary.csv / per_example_results.csv
        -- existing rows from the main run are never overwritten.

Output layout (mirrors fear_experiment.py):
  <output_dir>/fear_<name>/<completion_strategy>/
      sequence_rfm_directions.pt
      bag_of_words_rfm_directions.pt
      per_example_results.csv      (new rows appended)
      summary.csv                  (new rows appended)
...
def run_one_concept(fear, dataset_path, out_dir, args):
    ...
    # Mirror fear_experiment.py's strategy_tag logic (supports teacher source too)
    strategy_tag = (args.completion_strategy if args.completion_source == "self"
                    else f"{args.completion_strategy}_teacher")
    ...
    print(f"\n{'='*60}")
    print(f"FEAR: {fear}  |  STRATEGY: {args.completion_strategy}  |  SOURCE: {args.completion_source}")
    print(f"{'='*60}")
    ...
    fear_steer_and_evaluate.run_evaluation(
        dataset_path,
        baseline_path=None,           # skip mean_difference / rfm
...
def main(args):
    ...
    # Combined summary — only seqrfm / bag_of_words_rfm rows
    combined_path = os.path.join(
        args.output_dir,
        f"combined_summary_seqrfm_{strategy_tag}.csv"
    )
...
if __name__ == "__main__":
    ...
    p.add_argument("--completion_source",       default="self", choices=["self", "teacher"],
                   help="mirrors fear_experiment.py -- determines which strategy_tag "
                        "subfolder to read/write (self -> <strategy>, teacher -> <strategy>_teacher)")
