...
echo "############################################################"
python $CODE_DIR/persona_experiment.py \
    ...
    --seed                       42
    --train_prompt_suffix intentionally omitted here -- no length
    instruction on training prompts for this strategy.

echo ""
echo "Done. Per concept: outputs/$MODEL_TAG/persona_<name>/{short_instructed_cut,long_subsampled_anchor}/summary.csv"
echo "Combined across concepts: outputs/$MODEL_TAG/combined_summary_long_subsampled_anchor.csv"
echo "baseline_directions.pkl (mean_difference, rfm) is shared -- computed once, same file used by both strategies."
