#!/bin/bash
# run_seqrfm_personas.sh
#
# Runs ONLY sequence-RFM and bag-of-words RFM on the 30 persona concepts,
# both with truncate_at_end=True and fast_bow=True.
#
# Reuses existing artifacts (dataset_with_completions.json, trajectories.pt)
# from the long_subsampled_anchor run if they are already present, so
# steps 1-2 are skipped for any concept that is already done.
#
# New outputs written into the SAME strategy subfolder as the main run:
#   <OUT_DIR>/<MODEL_TAG>/persona_<name>/long_subsampled_anchor/
#       sequence_rfm_directions.pt
#       bag_of_words_rfm_directions.pt
#       per_example_results.csv   (sequence_rfm + bag_of_words_rfm rows)
#       summary.csv               (sequence_rfm + bag_of_words_rfm rows)
...
# ===============================
# HYPERPARAMETERS  (match run_completion_strategies_long.sh)
# ===============================
RFM_ITERS=3
SIGMA=1.0
LAM=1e-3
...
echo "# 30 persona concepts, long_subsampled_anchor"
echo "############################################################"

python $CODE_DIR/persona_experiment_seqrfm.py \
    --manifest_path            $DATA_DIR/personas/manifest30.json \
    --model                    $MODEL \
...
echo ""
echo "Done."
echo "Per-concept results: $OUT_DIR/$MODEL_TAG/persona_<name>/long_subsampled_anchor/summary.csv"
